...
1. Create a dedicated Google Cloud project and link the billing account. Set a project-scoped budget alert before running jobs.
...
6. After the demo, empty the report buckets and run `terraform destroy`. The buckets have `force_destroy = false` to guard against accidental report deletion. Delete the project after confirming no resources remain.
